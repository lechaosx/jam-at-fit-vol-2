...
@export var kaboomVelocityLimit: int = 500
@export var kaboomExplosionForce: float = 50000000
...
func kaboom():
	for body in $Area2D.get_overlapping_bodies():
		if body is RigidBody2D:
			...
			var force = kaboomExplosionForce / (4 * PI * distance * distance)
...
func _on_detection_area_body_entered(body: Node2D) -> void:
	...
	if velocity.length() > kaboomVelocityLimit:
		kaboom()
